...
exports.createadmUser = async function (req, res, next) {
    ...
    try {
        // Calling the Service function with the new object from the Request Body
        var createdUser = await AdminUserService.createUser(User)
        return res.status(201).json({createdUser, message: "Succesfully Created User"})
    } catch (e) {
        //Return an Error Response Message with Code and the Error Message.
        console.log(e)
...
exports.admloginUser = async function (req, res, next) {
    // Req.Body contains the form submit values.
    console.log("body",req.body)
    var User = {
        empid: req.body.empid,
    ...
    try {
        // Calling the Service function with the new object from the Request Body
        var admloginUser = await AdminUserService.admloginUser(User);
        return res.status(201).json({admloginUser, message: "Succesfully login"})
    } catch (e) {
        //Return an Error Response Message with Code and the Error Message.
        return res.status(400).json({status: 400, message: "Invalid username or password"})
    }
